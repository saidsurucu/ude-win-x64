# apply-imagefull.ps1 - satir-ici imaj tam-cozunurluk (bulaniklik fix)
...
$work=Join-Path $BuildDir '_imagefull'; $out=Join-Path $work 'out'; if (Test-Path $work){Remove-Item $work -Recurse -Force}; New-Dir $out
Write-Ok "ImageInsertPatch derleniyor + calistiriliyor"
& $javac --release 11 -encoding UTF-8 -cp "$jvs" -d $work (Join-Path $PSScriptRoot 'ImageInsertPatch.java')
if ($LASTEXITCODE -ne 0) { throw "ImageInsertPatch derlenemedi" }
& $java -cp "$work;$jvs" ImageInsertPatch $Jar $out
if ($LASTEXITCODE -ne 0) { throw "ImageInsertPatch calismadi" }
& $jarTool uf $Jar -C $out .
if ($LASTEXITCODE -ne 0) { throw "yamali sinif eklenemedi" }
Write-Ok "IMGFULL uygulandi"
