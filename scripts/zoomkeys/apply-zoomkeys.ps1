# apply-zoomkeys.ps1 - Ctrl+/Ctrl- klavye ile belge yakinlastirma
...
$work=Join-Path $BuildDir '_zoomkeys'; $hc=Join-Path $work 'helper'; $out=Join-Path $work 'out'
if (Test-Path $work){Remove-Item $work -Recurse -Force}; New-Dir $hc; New-Dir $out
Write-Ok "ZoomKeys yardimcisi derleniyor"
& $javac --release 11 -encoding UTF-8 -nowarn -d $hc (Join-Path $dir 'com\udewin\zoom\ZoomKeys.java')
if ($LASTEXITCODE -ne 0) { throw "ZoomKeys derlenemedi" }
& $jarTool uf $Jar -C $hc .
if ($LASTEXITCODE -ne 0) { throw "ZoomKeys jar'a eklenemedi" }
Write-Ok "ZoomPatch derleniyor + calistiriliyor"
& $javac --release 11 -encoding UTF-8 -cp "$jvs;$hc" -d $work (Join-Path $dir 'ZoomPatch.java')
if ($LASTEXITCODE -ne 0) { throw "ZoomPatch derlenemedi" }
& $java -cp "$work;$jvs;$hc" ZoomPatch $Jar $out
if ($LASTEXITCODE -ne 0) { throw "ZoomPatch calismadi" }
& $jarTool uf $Jar -C $out .
if ($LASTEXITCODE -ne 0) { throw "yamali sinif eklenemedi" }
Write-Ok "ZOOMKEYS uygulandi (Ctrl+/Ctrl-)"
